...
class Excute:
    ...
    def fifl(self, __o: Process, li: list) -> int:
        """
        the function name is find index from list \n
        arge : __o : 'Process' , li : 'list' \n
        return : int
        """
        for i in li:
            if i == __o:
    ...
    @property
    def calculate_FCFS(self, state=0, time_line=None):
        if state == 0:
        ...
        for item in self.process_list:
            if time_line < item.ProcessArriveTime:
                ...
                if state != 0:
                    item.ProcessExitTimeFromQueue.append(time_line)
                item.ProcessIsComplete = True
                item.ProcessRemainingTime = 0

        for item in self.process_list:
            self.total_waiting_time += int(item.ProcessWaitingTime)
            self.total_service_time += int(item.ProcessServiceTime)
            item.ProcessTurnaroundTime = float(
                item.ProcessWaitingTime) + float(item.ProcessArriveTime)
    ...
    def calculate_SJF(self) -> None:
        ...
        while True:
            ...
            if len(temp_process_time_line) > 0:
                temp_process_time_line.sort(
                    key=lambda e: e["ProcessServiceTime"])
                ...
                self.process_list[itptl] = current_process if itptl != - \
                    1 else self.process_list[itptl]
    ...
    def calculate_SRT(self) -> None:
        ...
        for process in self.process_list:
            wait = process.ProcessExitTimeFromQueue[0] - \
                process.ProcessServiceTime - \
                process.ProcessEnterTimeFromQueue[0]
    ...
    def calculate_RR(self) -> None:
        ...
        for item in temp_list:
            ...
            self.process_list[i_item] = item if i_item != -1 else self.process_list[i_item]
        ...
        while not is_complate:  # Start while
            ...
            if not curennt_process.is_null() and len(temp_process_time_line_list) != 0:
                # new Process arrived to first of list
    ...
    @property
    def calculate_MLFQ(self) -> None:
        self.sort_by_type(sort_type="mlfq")
        temp_list = [i for i in self.process_list]
        for i in temp_list:
            i.ProcessRemainingTime = i.ProcessServiceTime
            i.ProcessEnterTimeFromQueue.append(i.ProcessArriveTime)
            ind = self.fifpl(i)
            self.process_list[ind] = i if ind != -1 else self.process_list[ind]
        time_line = self.process_list[0].ProcessArriveTime
        get_q_time_list = [1 for i in range(len(self.process_list))]
        temp_time_line_list = []
        cureent_process = Process()
        temp_process = Process()
        is_complate = False
        temp_time_line_list = temp_time_line_list[::-1]
        for process in temp_list[::-1]:
            process : Process
            if process.ProcessArriveTime <= time_line:
                if process not in temp_time_line_list:
                    temp_time_line_list.append(process)
        temp_time_line_list = temp_time_line_list[::-1]
        mmad = False
        while not is_complate:
            if len(temp_time_line_list) != 0:
                cureent_process:Process = temp_time_line_list[0]
                push_time_lime = get_q_time_list[cureent_process.ProcessID]
            if not cureent_process.is_null() and cureent_process.ProcessRemainingTime <= get_q_time_list[cureent_process.ProcessID]:
                push_time_lime = cureent_process.ProcessRemainingTime
                cureent_process.ProcessRemainingTime = 0
                cureent_process.ProcessIsComplete = True
                cureent_process.ProcessExitTimeFromQueue.append(time_line + push_time_lime)
                temp_time_line_list.pop(0)
                temp_list.remove(cureent_process)
                mmad = True
                if len(temp_time_line_list) == 0:
                    is_complate = True
                    break
            else:
                get_q_time_list[cureent_process.ProcessID] *= 2
            if not cureent_process.is_null():
                cureent_process.ProcessRemainingTime -= push_time_lime
                ind = self.fifpl(cureent_process)
                self.process_list[ind] = cureent_process if ind != -1 else self.process_list[ind]
                if mmad != True:
                    # process add to first in list
                    temp_process = temp_time_line_list[0]
                    temp_time_line_list.pop(0)
                    temp_time_line_list.append(temp_process)
                    
                    temp_time_line_list = temp_time_line_list[::-1]
                    for process in temp_list[::-1]:
                        process : Process
                        haghgo = True if len(self.process_list) <= self.process_list[-1].ProcessArriveTime else False
                        if haghgo:
                            if process.ProcessArriveTime <= time_line + push_time_lime:
                                if process not in temp_time_line_list:
